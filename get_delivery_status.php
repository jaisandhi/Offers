...
function get_data()
{
	...
	window.location.href = "get_delivery_status.php?filter_date="+a+"&location="+b;
}
</script>
<style>
.marquee_widths
...
<b><span style="color:#005E7E;"><br>Update courier status</span></b>
<br>
<br>
...
<br>
<input type="button" value="Submit" onclick="get_data()"/> <input type='button' onclick='form_submit("0","move_all_to_delivery");' value='Move all to delivery'/> <input type="button" value="Refresh" onclick="redirect_indelivery()"/> 
...
{
$location_filter="a.location=b.id_loc and a.sale_delivery_date like'%".$_REQUEST['filter_date']."%' and a.is_deleted=0";	
}
else if($location_filter!=0 && $filter_date!="")
{
$location_filter="a.location=b.id_loc and a.sale_delivery_date like'%".$_REQUEST['filter_date']."%' and a.location=".$location_filter." and a.is_deleted=0";		
}
else if($location_filter==0 && $filter_date=="")
{
$location_filter="a.location=b.id_loc and a.location=b.id_loc and a.sale_stat=2 and a.is_deleted=0";		
}
...
{
$query = "select a.*,b.location from sales a , location_price b where a.location=b.id_loc and a.sale_stat=2 and a.is_deleted=0";
}

$result = $link->query($query) or die("Error in the consult." . mysqli_error($link));
$row_count=mysqli_num_rows($result);
echo "<b>Total new orders:&nbsp;".$row_count."</b>
<br><br>
<table class='table table-bordered table-striped'>
<tr>
<td>Action</td><td>Country</td><td>Name</td><td>Mobile</td><td>Address</td><td>Date</td></tr>";
while($res=mysqli_fetch_assoc($result))
{
echo "
<tr><td><input type='button' class='header_back btn btn-info btn-lg' onclick='form_submit(\"".$res['sales_id']."\",\"success_delivery\");' value='Delivered' style='".$disp."' />&nbsp;&nbsp;&nbsp;<input type='button' class='header_back btn btn-info btn-lg' onclick='form_submit(\"".$res['sales_id']."\",\"delivery_cancel\")' value='Delivery cancel' style='".$disp."' /></td><td>".$res['location']."</td><td>".$res['name']."</td><td>".$res['mobile']."</td><td>".$res['address']."</td><td>".$res['sale_delivery_date']."</td></tr>";
}
echo "</table>";
?>
