...
<td colspan="2">
<center>
<input type="button" value="Submit" onclick="get_data()"/> <input type='button' onclick='form_submit("0","move_all_to_open");' value='Move all' style="<?php echo $disp;?>"/> <input type="button" value="Refresh" onclick="redirect_liveorders()"/> </center>
</td>
</tr>
</table>
</div>
...
{
$location_filter="a.location=b.id_loc and a.created_date like'%".$_REQUEST['filter_date']."%' and a.is_deleted=0";	
}
else if($location_filter!=0 && $filter_date!="")
{
$location_filter="a.location=b.id_loc and a.created_date like'%".$_REQUEST['filter_date']."%' and a.location=".$location_filter." and a.is_deleted=0";		
}
else if($location_filter==0 && $filter_date=="")
{
$location_filter="a.location=b.id_loc and a.location=b.id_loc and a.sale_stat=0 and a.is_deleted=0";		
}
$query = "select a.*,b.id_loc as loc_id, b.location from sales a , location_price b where a.location=b.id_loc and ".$location_filter;
$date_data="on ".$_REQUEST['filter_date']."";
$disp="display:none;";
}
else
{
$query = "select a.*,b.id_loc as loc_id,b.location from sales a , location_price b where a.location=b.id_loc and a.sale_stat=0 and a.is_deleted=0";
...
$init_i++;
$assign_vals="";
if($res['assign_id']=="" || $res['assign_id']==0)
{
$assign_vals=get_loc_names($res['loc_id'],$init_i)."</br><input type='button' class='' value='Assign' onclick='form_assign(\"".$res['sales_id']."\",\"assign\",\"".$init_i."\");'/>";
}
else
{
$assign_vals="Assigned already";
}
...
<tr><td class='backred' style='color:#fff;'>Name</td><td class='backblue' style='background:#B0DED4 !important;'>".$res['name']."</td></tr>
...
<tr><td class='backred' style='color:#fff;'>Assign </td><td style='background:#B0DED4 !important;'>".$assign_vals."</td></tr>
<tr><td colspan='2'><input type='button' class='' onclick='form_submit(\"".$res['sales_id']."\",\"open\");' value='Move to open' style='".$disp."' />&nbsp;&nbsp;&nbsp;<input type='button' class='' onclick='form_submit(\"".$res['sales_id']."\",\"fake\")' value='Fake' style='".$disp."' /></td></tr>
</table>
</div>";
}
