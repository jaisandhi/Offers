...
<script>
    ...
    function get_data()
    {
        ...
        window.location.href = "delivery_reports.php?filter_month="+a+"&filter_year="+b+"&location="+c+"&user_loc_filter="+d;
    }
</script>
<style>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        var url="getsalespersondata.php?filter_month="+ans+"&filter_year="+ans1+"&location="+bloc+"&user_loc_filter="+bloc1;
        ...
        $.getJSON(url, function (result) {
            ...
            {
            ?>
            $.getJSON("getsalespersondata.php", function (result) {
                <?php
                }
                ?>
...
<div class="panel-body" style=" -moz-box-shadow: -1px 5px 17px #262526;    -webkit-box-shadow: -1px 5px 17px #262526;    box-shadow: -1px 5px 17px #262526;">
            ...
            <tr>
                ...
                <td style="background:#B0DED4 !important;">
                    <select id="location_filter">
                        <?php
                        $query_0 = "select * from locations b where b.is_deleted=0";
                        ...
                        {
                            echo "<option value='".$res_0['loc_id']."'>".$res_0['loc_name']."</option>";
                        }
    ...
    <div style=" margin:0px auto; overflow-x:auto;">
        ...
        if(isset($_REQUEST['filter_month']) && isset($_REQUEST['filter_year']) && isset($_REQUEST['user_loc_filter']))
        {
            ...
            {
                $query = "SELECT u.user_name,l.loc_name as location,count(p.assign_id) as assign_orders,sum(p.sale_stat = 3) as delivered,
                sum(p.sale_stat = 4) as canceled FROM login u JOIN sales p JOIN locations l WHERE l.loc_id = u.location AND 
                l.loc_id = p.location AND p.assign_id = u.log_id AND u.role=3 AND u.location=$location_filter AND
            ...
            else if($location_filter!=0 && $user_location_filter!=0  && $filter_date=="" && $filter_date1=="")
            {
                $query = "SELECT u.user_name,l.loc_name as location,count(p.assign_id) as assign_orders,sum(p.sale_stat = 3) as delivered,
                sum(p.sale_stat = 4) as canceled FROM login u JOIN sales p JOIN locations l WHERE l.loc_id = u.location AND 
                l.loc_id = p.location AND p.assign_id = u.log_id AND u.role=3 AND u.location=$location_filter AND u.log_id=$user_location_filter AND
                u.is_deleted=0 GROUP BY u.log_id";
                $result = $link->query($query) or die("Error in the consult." . mysqli_error($link));
            }
            else if($location_filter==0 && $user_location_filter==0 && $filter_date!="" && $filter_date1=="")
            {
                $query = "SELECT u.user_name,l.loc_name as location,count(p.assign_id) as assign_orders,sum(p.sale_stat = 3) as delivered,
                sum(p.sale_stat = 4) as canceled FROM login u JOIN sales p JOIN locations l WHERE l.loc_id = u.location AND 
                l.loc_id = p.location AND p.assign_id = u.log_id AND u.role=3 AND MONTH(p.created_date) = $filter_date
            ...
            else if($location_filter==0 && $user_location_filter==0 && $filter_date!="" && $filter_date1!="")
            {
                $query = "SELECT u.user_name,l.loc_name as location,count(p.assign_id) as assign_orders,sum(p.sale_stat = 3) as delivered,
                sum(p.sale_stat = 4) as canceled FROM login u JOIN sales p JOIN locations l WHERE l.loc_id = u.location AND
                l.loc_id = p.location AND p.assign_id = u.log_id AND u.role=3 AND MONTH(p.created_date) = $filter_date AND  Year(p.created_date) = $filter_date1
                 AND u.is_deleted=0 GROUP BY u.log_id";
            ...
            else if($location_filter==0 && $user_location_filter==0 && $filter_date=="" && $filter_date1!="")
            {
                $query = "SELECT u.user_name,l.loc_name as location,count(p.assign_id) as assign_orders,sum(p.sale_stat = 3) as delivered,
                sum(p.sale_stat = 4) as canceled FROM login u JOIN sales p JOIN locations l WHERE l.loc_id = u.location AND
                l.loc_id = p.location AND p.assign_id = u.log_id AND u.role=3 AND Year(p.created_date) = $filter_date1
                 AND u.is_deleted=0 GROUP BY u.log_id";
                $result = $link->query($query) or die("Error in the consult." . mysqli_error($link));
            }
            else if($location_filter!=0 && $user_location_filter==0 && $filter_date!="" && $filter_date1!=""){
                ...
                $query = "SELECT u.user_name,l.loc_name as location,count(p.assign_id) as assign_orders,sum(p.sale_stat = 3) as delivered,
                sum(p.sale_stat = 4) as canceled FROM login u JOIN sales p JOIN locations l WHERE l.loc_id = u.location AND
                l.loc_id = p.location AND p.assign_id = u.log_id AND u.role=3 AND u.location=$location_filter AND MONTH(p.created_date) = $filter_date AND
                ...
                $query = "SELECT u.user_name,l.loc_name as location,count(p.assign_id) as assign_orders,sum(p.sale_stat = 3) as delivered,
                sum(p.sale_stat = 4) as canceled FROM login u JOIN sales p JOIN locations l WHERE l.loc_id = u.location AND 
                l.loc_id = p.location AND p.assign_id = u.log_id AND u.role=3 AND u.is_deleted=0 GROUP BY u.log_id";
                $result = $link->query($query) or die("Error in the consult." . mysqli_error($link));
            }
        }
        else
        {
//            $query = "SELECT u.user_name,group_concat(p.name separator ',') as salesname ,l.loc_name as location,count(p.assign_id) as assign_orders,sum(p.sale_stat = 3) as delivered,
//            sum(p.sale_stat = 4) as canceled FROM login u JOIN sales p JOIN locations l WHERE l.loc_id = u.location AND
//            l.loc_id = p.location AND p.assign_id = u.log_id AND u.role=3 AND u.is_deleted=0 GROUP BY u.log_id";
        ...
        {
//            $val.="<tr><td>".$res['user_name']."</td><td>".$res['location']."</td><td>".$res['salesname']."</td><td>".$res['assign_orders']."</td><td>".$res['delivered']."</td><td>".$res['canceled']."</td></tr>";
            $val.="<tr><td>".$res['user_name']."</td><td>".$res['location']."</td><td>".$res['assign_orders']."</td><td>".$res['delivered']."</td><td>".$res['canceled']."</td></tr>";
        }
...
//            {
//                $date_data="<b>Total Sales Person:&nbsp;".$sum_val;
//            }
//            else if($location_filter==0 && $filter_date!="" && $filter_date1=="")
//            {
//                $date_data="<b>Total Sales Person:&nbsp;".$sum_val." in this month ".$filter_date;
//            }
//            else if($location_filter==0 && $filter_date=="" && $filter_date1!="")
//            {
//                $date_data="<b>Total Sales Person:&nbsp;".$sum_val." in this year ".$filter_date1;
//            }
//            else if($location_filter==0 && $filter_date!="" && $filter_date1!="")
//            {
//                $date_data="<b>Total Sales Person:&nbsp;".$filter_date." month  ".$filter_date1;
//            }
//            else
//            {
//                $date_data="<b>Total Sales Person:&nbsp;".$filter_date." month  ".$filter_date1;
...
//        else
//        {
//            $date_data="<b>Total :&nbsp;".$sum_val."</b> in ".date("Y")."<br>";
//        }
        ?>
        ...
        <div class='col-md-12'>
            ...
            <table class='table table-bordered table-striped'>
                <!--                <thead><tr style='text-align:center'><td>User Name </td><td>Location</td><td>Sales Name</td><td>Total Assigned Orders</td><td>Total Delivered Count</td><td>Total Cancelled Count</td></tr></thead>-->
                <thead><tr style='text-align:center'><td>User Name </td><td>Location</td><td>Total Assigned Orders</td><td>Total Delivered Count</td><td>Total Cancelled Count</td></tr></thead>
                <?php
                echo $val;
                ?>
            </table>
